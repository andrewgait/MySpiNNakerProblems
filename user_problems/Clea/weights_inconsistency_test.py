...
def make_network(rate, start_time,duration ):
    ...
    # seed = 0
    source_pop = sim.Population(25, source_pop_obj) #, additional_parameters={"seed": seed})

    excit_pop = sim.Population(9, sim.IF_cond_exp(**excit_param), label='excitatory',
                               additional_parameters={'splitter': post_spliter})  #, 'seed': seed})
    ...
    # timing_stdp = sim.SpikePairRule(tau_plus=20, tau_minus=20, A_plus=0, A_minus=0.0001)
    # weight_stdp = sim.AdditiveWeightDependence(w_min=0, w_max=1.0)
    ...
    stdp_model_triplet = sim.STDPMechanism(timing_dependence=timing_stdp, weight_dependence=weight_stdp,
                                         weight=RandomDistribution('normal_clipped_to_boundary', mu=0.1, sigma=0.1, low=0, high=10, rng=numpy_RNG),
    ...
    E2I_projec = sim.Projection(excit_pop, inhit_pop, sim.OneToOneConnector(), receptor_type="excitatory",
    ...
    return excit_pop, inhit_pop, input_projec, E2I_projec, I2E_projec
...
print("rate_gen_np, rate_gen_trans ", len(rate_gen_trans), len(rate_gen_trans[12]), rate_gen_trans[0][38], rate_gen_trans[12][38], rate_gen_trans[24][38])
start_time = [500*i for i in range(40)]
duration = [350]*40
excit_pop, inhit_pop, stdp_projec, e2i, i2e = make_network(rate_gen_trans[:, :40], start_time, duration)
...
print("rearranged: ", rearrange_w)
spike = excit_pop.get_data('spikes')

e2i_weight = e2i.get(["weight"], "array")
i2e_weight = i2e.get(["weight"], "array")

print("e2i weight ", e2i_weight)
print("i2e weight ", i2e_weight)

print("spikes: ", spike.segments[0].spiketrains)
...
fig3 = plt.figure(3)
im3 = plt.imshow(weight, interpolation="nearest", vmin=0, cmap=chosenCmap)
plt.colorbar(im3)
plt.title('weights before rearranging ')
fig3.canvas.draw()

# fig4 = plt.figure(4)
# im4 = plt.imshow(e2i_weight, interpolation="nearest", vmin=0, cmap=chosenCmap)
# plt.colorbar(im4)
# plt.title('e2i weights ')
# fig4.canvas.draw()
#
# fig5 = plt.figure(5)
# im5 = plt.imshow(i2e_weight, interpolation="nearest", vmin=0, cmap=chosenCmap)
# plt.colorbar(im5)
# plt.title('i2e weights ')
# fig5.canvas.draw()
...
#     Panel(spike.segments[0].spiketrains, xlabel="Time/ms", xticks=True,
#           yticks=True, markersize=0.2, xlim=(0, t_one_sample*40)),
#     title="excit spikes",
