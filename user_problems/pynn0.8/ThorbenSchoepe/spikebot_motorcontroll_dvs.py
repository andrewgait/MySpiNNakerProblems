...
def get_updated_params(params):
	params.update({"spinnaker_link_id": 1})
	return params
...
class Populations:
	...
	retina = p.Population( Size.retina, p.external_devices.ExternalFPGARetinaDevice, get_updated_params({
	'retina_key': 0x200,
	'mode': p.external_devices.ExternalFPGARetinaDevice.MODE_64,
	'polarity': p.external_devices.ExternalFPGARetinaDevice.MERGED_POLARITY}),
...
class Connections:
	...
	sptc_on = [(int(retina[i]+44*Size.retina_edge),int(SPTC_connection[i]),Weights.sptc,1) for i in range(Size.retina_connections)]
