...
moving straight output neuron ids: 85, 328 (328 = 1.285ms)
...
path_spikes = path_plots = '/home/spinnaker/Documents/projects/motorcontroll_spikebot/spikebot_corridor_2021_01/'
...
def rasterplot(spiketimes,populations_size, simtime, x_labels, y_labels, path_save, plot_type, bin_size):
	...
	for num in range(1, number_subplots+1):
		...
		if plot_type[num-1] == 'rasterplot':
			...
			spikes = []
			for i in range(len(spiketimes[num-1].segments[0].spiketrains)):
				spikes = np.append(spikes, spiketimes[num-1].segments[0].spiketrains[i])
			print spikes
...
def input_live():
	...
	#retina_resolution = p.external_devices.PushBotRetinaResolution.DOWNSAMPLE_32_X_32 
...
p.set_number_of_neurons_per_core(p.IF_curr_exp_sEMD, 100)  # was 100 for SpiNN3
#p.set_number_of_neurons_per_core(p.IF_curr_exp, 100)  # was 100 SpiNN3
...
#-- Config parameters --#
t               =   20000                 #time of simulation in ms
...
class Populations:
	...
	spike_3 = p.Population(1, p.SpikeSourceArray, {'spike_times': [[]]}, label = "input_pop")
	spike_1 = p.Population(1, p.SpikeSourceArray, {'spike_times': [[1000]]}, label = "input_pop")
	spike_4 = p.Population(1, p.SpikeSourceArray, {'spike_times': [[]]}, label = "input_pop")
	spike_2 = p.Population(1, p.SpikeSourceArray, {'spike_times': [[8000]]}, label = "input_pop")
	...
	# sEMD populations
	semd_rl = p.Population(Size.semd,p.IF_curr_exp_sEMD,CellParams.sEMD,label='sEMD_pop_right_left')
	semd_lr = p.Population(Size.semd,p.IF_curr_exp_sEMD,CellParams.sEMD,label='sEMD_pop_left_right')
	# Integrator neurons
...
SPTC_connection = np.arange(Size.sptc_connections)
...
SPTC_connection.resize(Size.retina/2)
...
class Connections:
	...
	oszillator_to_output_1 = [[i, 256+turn+correction_factor, 10, 1] for i in range(0,Size.oszillator_pop,8)] # 
	oszillator_to_output_2 = [[i,turn+correction_factor_2,10,1] for i in range(0,Size.oszillator_pop,8)] 
	oszillator_to_output_3 =[[i,256+turn+correction_factor,10,1] for i in range(4,Size.oszillator_pop,8) ] 
	...
	oszillator2_to_output_2 = [[i,256+turn+correction_factor,10,1] for i in range(0,Size.oszillator_pop,8)]
	oszillator2_to_output_3 =[[i,turn+correction_factor_2,10,1] for i in range(4,Size.oszillator_pop,8) ] 
	oszillator2_to_output_4 =[[i,256+turn+correction_factor,10,1] for i in range(4,Size.oszillator_pop,8) ]
...
# moving straight
p.Projection(Populations.oszillator_1, Populations.output, p.FromListConnector([, [Size.oszillator_pop-1,turn+correction_factor_2,10,3]]), p.StaticSynapse(), receptor_type='excitatory')# was +80 and 80
p.Projection(Populations.oszillator_2, Populations.output, p.FromListConnector([, [Size.oszillator_pop-1,turn+correction_factor_2,10,3]]), p.StaticSynapse(), receptor_type='excitatory')# was +80 and 80
...
#p.Projection(Populations.oszillator_1, Populations.efference_copy, p.AllToAllConnector(), p.StaticSynapse(weight=1,delay=1), receptor_type='excitatory')
#p.Projection(Populations.oszillator_2, Populations.efference_copy, p.AllToAllConnector(), p.StaticSynapse(weight=1,delay=1), receptor_type='excitatory')
#p.Projection(Populations.efference_copy, Populations.sptc, p.AllToAllConnector(), p.StaticSynapse(weight=30,delay=1), receptor_type='inhibitory')
...
spikedata = [ Spikes.wta, Spikes.oszillator1, Spikes.oszillator2, Spikes.integrator_lr, Spikes.integrator_rl, Spikes.output]
size_populations = [Size.wta, Size.oszillator_pop,  Size.oszillator_pop, Size.semd_edge_horizontal, Size.semd_edge_horizontal, Size.output]
y_labels = ['wta, low:l, high:r', 'Oszi left', 'Oszi right', 'sEMD lr', 'sEMD rl', 'out']
plot_type = ['rasterplot','rasterplot', 'rasterplot','rasterplot', 'rasterplot', 'rasterplot']
