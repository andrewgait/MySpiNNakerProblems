...
// 0x0040 5c00 Circular Buffer for FIQ (size  1,024 bytes;  1 KB; 256 in spikes)
// 0x0040 6000 DMA buffer 1            (size  6,144 bytes;  6 KB; 1.5K synapses)
// 0x0040 7800 Random Number Buffer    (size  2,048 bytes;  2 KB; 512 random numbers)
    ...
    for (i = 0; i < 256; i++)
	    ...
	    if (ring_buffer[(256*j + i) * 2 + buffer_number] != 0xffff)
                io_printf (IO_BUF, "ring_buffer%1u [%d, %u] = %u\n",
                           buffer_number, j, i, 0xffff - (uint32_t)ring_buffer[(256*j + i)*2+buffer_number]);
}
...
void translate_tmp (void) __attribute__ ((noinline));
...
void new_modelling_time_step      (void)  asm ("new_modelling_time_step")     __attribute__ ((noinline, naked));
...
//----------------------------------------------------------------------------------------------------------
	     ...
	     (uint32_t)dma_empty,           // 128 DMA Processing Address
	     0x60800000,                    // 132 SDRAM base address for this processor (p) is 0x60000000 + p * 0x800000
	     0,                             // 136 Saturation Counter
	     0xa0000000,                    // 140 V_reset value
	     0x50000000,                    // 144 Refractory counter initial value
...
#define global_timer_tick()						                                                \
  do {asm volatile ("ldr  r0, [%[ctrl], %[lo]]  @ Load the low word of the timer                                  \n\t"	\
		    ...
		    "adds r1, r1, #0            @ increment high word (absorbing carry flag if necessary)         \n\t" \
...
#define time_base_tick()						                                                  \
  do {asm volatile ("ldr  r0, [%[ctrl], %[time]]  @ Load the time_base word                                         \n\t" \
		    "adds r0, r0, #0x10000000     @ increment time_base word                                        \n\t" \
...
#define feed_dma_if_needed()						                                                   \
    do {asm volatile ("mov    r3, #0x40000000           @ Load base-address of DMA register base                     \n\t" \
		      "ldr    r2, [r3, %[status]]       @ Check DMA status                                           \n\t" \
		      "ldr    r0, [%[ctrl], %[addr]]    @ Load address of an outgoing DMA processing routine         \n\t" \
		      "ands   r2, r2, #0xc00            @ Test whether there are any spare DMA slots, this occurs .. \n\t" \
...
/*
    ldr   r2, [%[wp]], #4               @ Load the rowlet descriptor word (RDW)                         \n\t" \
@ ----------------->                    @ pipeline interlock here                                 \n\t" \
    and   r0, %[mask_0x3c], r2, lsr #5  @ Mask out delay bits                               \n\t" \
    ldr   r1, [%[ctrl], r0]	        @ Load rowlet buffer pointer                              \n\t" \
@ ----------------->                    @ pipeline interlock here                                 \n\t" \
    str   r2, [r1], #4                  @ Store RDW in buffer                                  \n\t" \
    str   r1, [%[ctrl], r0]	        @ Write-back the rowlet buffer pointer                    \n\t" \
		      	      
*/
#define rowlet_extension()						                                                   \
    do {asm volatile ("ldr   r2, [%[wp]], #4               @ Load the rowlet descriptor                              \n\t" \
		      "@ ----------------->                @ pipeline interlock here                                 \n\t" \
		      "and   r3, %[mask_0x3c], r2, lsr #5  @ Mask out all but bits 2-5                               \n\t" \
