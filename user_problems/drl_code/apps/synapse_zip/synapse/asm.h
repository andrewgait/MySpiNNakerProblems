...
// #define TIME_CONST_V = 20.0

// 1 - exp (-0.1/0.5)   ~= 11880/65536
// 1 - exp (-0.1/20.0)  ~=  652/65536

//#define signed_16bit(x) (((x < 0)? (0x10000 -(x)): (x)))
// tau = RC
// 20 = tau; C = 250pF; R = 80,000 MOhm?


#define PSC_COEFFICIENTS     ((0 << 16) | (0x10000 - 11880))
#define VOLTAGE_COEFFICIENTS ((0 << 16) | (0x10000 -   652))
#define PHI_B_COEFFICIENT    16184567  /*phi =   (V_RESET_VOLTAGE-phi) * (1 -  163 / 32768) */
#define V_RESET_VOLTAGE      0xa0000000
...
#define RDW_BASE             (MASTER_POP_BASE + MASTER_POP_SIZE)
#define RDW_BUFFER_SIZE      (256*4)
#define ROWLET_BASE          (RDW_BASE + 16 * RDW_BUFFER_SIZE)
...
//----------------------------------------------------------------------------------------------------------
	     ...
	     (uint32_t)neuron,              // 152 Neurons variables address
...
#define feed_dma_if_needed()						                                                   \
    do {asm volatile ("ldr    r0, [%[ctrl], %[addr]]    @ Load address of an outgoing DMA processing routine         \n\t" \
		      "mov    r3, #0x40000000           @ Load base-address of DMA register base                     \n\t" \
		      "ldr    r2, [r3, %[status]]       @ Check DMA status                                           \n\t" \
		      "cmp    r0, #0                    @ Test dma_processing address; if 0 then DMA requests empty  \n\t"\
		      "andnes r2, r2, #0xc00            @ Test whether there are any spare DMA slots, this occurs .. \n\t" \
		      "                                 @   .. if either of the DMA-completed flags are set          \n\t" \
		      "blxne  r0                        @ Set up new DMA(s), provided there are slots and requests   \n\t" \
