"""
Attempting to rewrite the IF_curr_dual_exp example from PyNN7Examples for pyNN 0.8;
for some reason the PyNN8Examples version is completely different... ?
"""
...
for i in range(n_pop):
    if i == 0:
        ...
        populations[i].initialize(v=v_distr)
    elif i == 1:
        populations.append(p.Population(
    ...
    if i == 1:
        projections.append(p.Projection(
            ...
            p.StaticSynapse(weight=weight_to_spike, delay=10)))  # , target='excitatory'))
        projections.append(p.Projection(
            populations[i], dual_stim_population, p.OneToOneConnector(),
            p.StaticSynapse(weight=weight_to_spike, delay=10)))  # , target='excitatory2'))
    if i > 0:
        projections.append(p.Projection(
...
for i in range(n_pop):
    ...
    data = numpy.asarray(populations[i].spinnaker_get_data('spikes'))
    # data = numpy.asarray(populations[i].get_data('spikes').segments[0].spiketrains)
    if len(data) > 0:
        py_plot.scatter(
