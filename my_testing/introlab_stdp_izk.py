...
pre_pop = sim.Population(n_neurons, sim.Izhikevich(), label="Pre")
post_pop = sim.Population(n_neurons, sim.Izhikevich(), label="Post")
...
sim.Projection(training, pre_pop,  sim.OneToOneConnector(),
               synapse_type=sim.StaticSynapse(weight=10.0, delay=1.0))
sim.Projection(training, post_pop, sim.OneToOneConnector(),
               synapse_type=sim.StaticSynapse(weight=10.0, delay=10.0))

timing_rule = sim.SpikePairRule(tau_plus=20.0, tau_minus=20.0,
                                A_plus=0.5, A_minus=0.5)
weight_rule = sim.AdditiveWeightDependence(w_max=10.0, w_min=0.0)
