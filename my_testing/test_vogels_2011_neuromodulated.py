...
class Vogels2011(object):
    ...
    def _build_network(self, uses_stdp, slow_down):
        ...
        # Add neuromodulation to synapses (has to be done in this order)
        if uses_stdp:
            rewards = [x for x in range(200, 210)] + \
                      [x for x in range(300, 320)] + \
                      [x for x in range(400, 410)]
            punishments = [x for x in range(600, 610)] + \
                          [x for x in range(700, 720)] + \
                          [x for x in range(800, 810)]
            # Create a population of dopaminergic neurons for reward and punishment
            reward_pop = sim.Population(self.NUM_EXCITATORY, sim.SpikeSourceArray,
                                        {'spike_times': rewards}, label='reward')
            punishment_pop = sim.Population(self.NUM_EXCITATORY, sim.SpikeSourceArray,
                                            {'spike_times': punishments},
                                            label='punishment')
            sim.Projection(reward_pop, ex_pop, sim.OneToOneConnector(),
            ...
            sim.Projection(punishment_pop, ex_pop, sim.OneToOneConnector(),
                           synapse_type=sim.extra_models.Neuromodulation(
                               weight=0.05, tau_c=100.0, tau_d=5.0, w_max=20.0),
    ...
    def plot(
        ...
        if plastic_spikes_numpy is not None:
            ...
            # only plot last 50th
            axes[1].set_xlim(
                self.SECOND_RUN_RUNTIME - (40 * self.SECOND_RUN_RUNTIME / 50),
