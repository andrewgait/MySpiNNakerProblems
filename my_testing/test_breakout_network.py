...
breakout_pop = p.Population(32768, p.SpikeSourceArray(**spikeArray),
                            label='fake_breakout')

# Connect key spike injector to breakout population
# key_input = p.Population(2, p.SpikeSourcePoisson(rate=1), label="key_input")
...
receive_pop = p.Population(receive_pop_size, p.IF_cond_exp(),
                           label="receive_pop")
print('len(Connections_on) = ', len(Connections_on))
p.Projection(breakout_pop, receive_pop,
...
d_conn.add_database_callback(functools.partial(
    start_visualiser, pop_label=breakout_pop.label, xr=x_factor1, yr=y_factor1,
